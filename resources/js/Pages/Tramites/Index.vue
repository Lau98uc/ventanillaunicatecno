  ...
  <div>
    ...
    <div class="flex items-center p-2 md:p-4 justify-between gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-sm font-bold">Tramites</h1>
      </div>
      ...
      <!-- Derecha: Paginación -->
      <div>
        <pagination :links="tramites.links">
          {{ tramites.from }}-{{ tramites.to }}/{{ tramites.total }}
        </pagination>
      </div>
    </div>
      ...
      <table class="table table-xs table-zebra w-full whitespace-nowrap">
        <thead class="sticky top-0 bg-base-200 z-10">
          <tr class="text-left font-bold">
            ...
            <th class="pb-4 pt-6 px-6" colspan="1">Workflow</th>
            <th class="pb-4 pt-6 px-6" colspan="2">Costo</th>

          </tr>
        </thead>
        <tbody>
          <tr v-for="tramite in tramites.data" :key="tramite.id" class="hover">

            <td class="px-6 py-3 border-t">
              <Link :href="`/tramites/${tramite.id}/edit`" tabindex="-1">
              {{ tramite.nombre }}
              </Link>
            </td>
            <td class="px-6 py-3 border-t">
          ...
          <tr v-if="tramites.data.length === 0">
            <td class="px-6 py-3 border-t" colspan="6">No hay tramites.</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/tramites', pickBy(this.form), { preserveState: true })
      }, 150)
    }
  },
  methods: {
