          ...
          <div class="pb-8 pr-6 w-full">
            ...
            <div class="max-h-[400px] overflow-y-auto border p-4 rounded space-y-4">
              ...
              <div v-for="modulo in permisos" :key="modulo.id" class="mb-4">
                ...
                <label class="inline-flex items-center text-sm cursor-pointer">
                  <input type="checkbox" class="checkbox checkbox-xs checkbox-accent" :value="modulo.id"
                    v-model="form.permissions" />
                  <span class="ml-2">Módulo</span>
                </label>
                ...
                <div v-for="menu in modulo.children" :key="menu.id" class="ml-4 mb-2">
                  <div class="flex items-center justify-between">
                    ...
                    <!-- Checkbox para el modelo -->
                    <label class="inline-flex items-center text-sm cursor-pointer">
                      <input type="checkbox" class="checkbox checkbox-xs checkbox-accent" :value="menu.id"
                        v-model="form.permissions" />
                      <span class="ml-2">Modelo</span>
                    </label>

                    <!-- Checkbox para marcar todas las acciones -->
                    <label class="inline-flex items-center text-sm cursor-pointer">
                      <input type="checkbox" class="checkbox checkbox-xs checkbox-secondary"
                        :checked="isMenuFullyChecked(menu)" @change="toggleMenuPermissions(menu)" />
                      <span class="ml-2">Todos</span>
  ...
  data() {
    return {
      form: this.$inertia.form({
        name: this.role.name,
        permissions: [...this.permisosAsignados],
      }),
    }
  },
  methods: {
